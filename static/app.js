...
async function loadScans() {
    try {
        ...
        if (scans.length === 0) {
            scansList.innerHTML = '<p class="loading">No scans yet. Run your first scan!</p>';
            return;
        }
            ...
            <div class="scan-item" onclick="viewScan(${scan.id})">
                <div class="scan-item-info">
                    ...
                    <div class="scan-item-meta">
                        <strong>${scanners[scan.scanner] || scan.scanner}</strong> •
        ...
        if (scans.length > 0 && !currentScanId) {
            viewScan(scans[0].id);
        }
    } catch (error) {
...
function startPolling(scanId) {
    ...
    pollInterval = setInterval(async () => {
        try {
            ...
            resultsDisplay.parentElement.scrollTop = resultsDisplay.parentElement.scrollHeight;
    ...
    try {
        ...
        if (response.ok) {
            ...
            if (currentScanId === scanId) {
                document.getElementById('resultsDisplay').innerHTML = '<p class="loading">Run a scan to see results</p>';
                currentScanId = null;
            }
            loadScans();
        } else {
